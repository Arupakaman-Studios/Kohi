package com.arupakaman.kohi.data
...
class KohiSharedPrefs private constructor(private val mPrefs: SharedPreferences){
    ...
    var restoreOnBoot by mPrefs.data(false)

    var stopOnLowBattery by mPrefs.data(true)

    var restoreOnUnlock by mPrefs.data(false)
    ...
    var toggleClickCount by mPrefs.data(0)
    ...
    companion object{
        ...
        private var mPrefs: KohiSharedPrefs? = null

        fun getInstance(mContext: Context): KohiSharedPrefs {
            if (mPrefs == null)
                mPrefs = KohiSharedPrefs(mContext.getSharedPreferences("KohiSharedPrefs", Context.MODE_PRIVATE))
            return mPrefs!!
        }
